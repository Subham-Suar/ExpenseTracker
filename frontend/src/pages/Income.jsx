...
export default function Income() {
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
      ...
      >
        <div>
          <h1 className="text-3xl font-bold text-slate-100 tracking-tight">Income Analytics</h1>
          <p className="text-slate-400 mt-1">Track and analyze your revenue streams.</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="lg:col-span-1 space-y-6">
          <motion.div variants={itemVariants} className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-sm flex items-center gap-4">
          ...
          <motion.div variants={itemVariants} className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-sm flex items-center gap-4">
            <div className="p-4 bg-sky-400/10 text-sky-400 rounded-xl">
              <Award size={28} />
            </div>
            <div>
              <p className="text-sm font-medium text-slate-400">Top Revenue Source</p>
              <h3 className="text-xl font-bold text-slate-100 capitalize">{stats.topCategory}</h3>
            </div>
          </motion.div>
        </div>

        {/* Bar Chart - Monthly Income Trend */}
        <motion.div variants={itemVariants} className="lg:col-span-2 bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-sm min-h-[350px] flex flex-col">
          <div className="flex items-center gap-2 mb-6">
            <BarChart3 className="text-green-500" size={24} />
            <h3 className="font-semibold text-slate-100 text-lg">
              Monthly Income Trends
              <span className="text-slate-400 font-normal text-sm ml-2 tracking-wide">(This Year)</span>
            </h3>
          </div>
        ...
        <motion.div 
          ...
          className="lg:col-span-3 bg-gradient-to-br from-slate-800 to-slate-700/50 p-8 rounded-2xl border border-slate-700 shadow-lg hover:shadow-xl transition-all duration-300 grid grid-cols-1 md:grid-cols-2 gap-8 items-center"
        >
          <div>
            <motion.div
            ...
            >
              <h3 className="font-bold text-2xl text-slate-100 mb-2">Income Distribution</h3>
              <p className="text-sm text-slate-400 mb-8">A breakdown of where your money is coming from by category.</p>
            </motion.div>
              ...
              {stats.pieData.map((entry, index) => {
                ...
                return (
                  <motion.div 
                    ...
                    whileHover="hover"
                    className={`flex items-center gap-4 p-4 rounded-xl border transition-all duration-300 ${sourceIcon.bg} border-slate-600/30 hover:border-slate-500 hover:shadow-lg cursor-pointer group`}
                  >
                    {/* Icon */}
                    <div className={`flex-shrink-0 p-3 rounded-lg ${sourceIcon.bg} ${sourceIcon.text} group-hover:scale-110 transition-transform duration-300`}>
                    ...
                    <div className="flex-grow min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-slate-200 font-semibold capitalize">{entry.name}</span>
